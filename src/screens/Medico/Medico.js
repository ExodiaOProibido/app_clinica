...
// Funções auxiliares para formatar o endereço (NOVO)
const formatEndereco = (m) => {
 return `${m.logradouro}, ${m.numero}${
  m.complemento ? " - " + m.complemento : ""
 }`;
};

const formatCidade = (m) => {
 return `${m.bairro}, ${m.cidade}/${m.uf} - CEP: ${m.cep}`;
};
...
const handleDeactivate = () => {
 ...
 text: "Sim, Desativar", 
 style: "destructive",
 onPress: () => onDeactivate(medico.id) // ⬅️ Chama a função injetada
 }, 
 ]
 );
 };
 ...
 {/* SEÇÃO EXPANSÍVEL (Detalhes - Refatorado) */}
 {isExpanded && (
 ...
 <Text style={cardStyles.detailText}>
 Logradouro: {formatEndereco(medico)}
 </Text>
 <Text style={cardStyles.detailText}>
 Local: {formatCidade(medico)}
 </Text>
...
const Op1Screen = ({ navigation, medicos, onDeactivate }) => {
 ...
 const activeMedicos = medicos.filter(m => m.status !== false);
...
const sections = useMemo(
 ...
 renderItem={({ item }) => (
 <MedicoCard medico={item} navigation={navigation} onDeactivate={onDeactivate} />
 )}
